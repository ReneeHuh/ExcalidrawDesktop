...
describe("loadDocumentContentIntoEditor", () => {
  ...
  it("leaves the editor unchanged when scene restoration fails", async () => {
    const api = createApi();
    await expect(loadDocumentContentIntoEditor({
      api, fileName: "invalid.excalidraw", content: "invalid",
      loadScene: vi.fn(async () => { throw new Error("invalid scene"); }),
    })).rejects.toThrow("invalid scene");
    expect(api.updateScene).not.toHaveBeenCalled();
    expect(api.addFiles).not.toHaveBeenCalled();
  });

  it("does not apply a parser result after cancellation", async () => {
    const api = createApi();
    await expect(loadDocumentContentIntoEditor({
      api, fileName: "cancelled.excalidraw", content: "{}", isCancelled: () => true,
      loadScene: vi.fn(async () => ({ elements: [], appState: {}, files: {} })) as never,
