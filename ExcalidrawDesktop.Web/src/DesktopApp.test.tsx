...
describe("mounted editor and native bridge orchestration", () => {
  ...
  it("cancels an export when the editor unmounts and ignores a late renderer result", async () => {
    await edit("export-before-unmount");
    const render = deferred<Blob>();
    fixture.renderPng.mockReturnValueOnce(render.promise);
    const ownerWindow = host.ownerDocument.defaultView!;
    ownerWindow.fetch = vi.fn(async () => ({ ok: true }) as Response);
    const exportId = id();
    const origin = "https://export-9d32585b1ac74f489ef08b7a55d49570.excalidraw.local";
    await emit("image.exportRequested", {
      exportId, uploadUrl: `${origin}/_desktop/export/${exportId}`,
      maxDimension: 16384, maxBytes: 10000, scale: 1, padding: 1,
    });
    await act(async () => root.render(null));
    await act(async () => render.resolve(new Blob(["late-png"], { type: "image/png" })));
    expect(ownerWindow.fetch).not.toHaveBeenCalled();
    expect(events("image.exportFailed")).toHaveLength(0);
    expect(fixture.api.setToast).not.toHaveBeenCalled();
  });
});
