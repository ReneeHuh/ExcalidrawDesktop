...
public sealed partial class MainWindow
{
    ...
    private async Task RunMultiWindowSmokeAsync()
    {
        ...
        try
        {
            ...
            // Initialization selects each tab in turn. Explicitly select the other
            // drawing and let its ready-message dispatch finish before testing idle rules.
            DocumentTabs.SelectedItem = sourceSession.TabItem;
            if (!await AsyncWait.UntilAsync(
                    () => editorSessions.CanSuspend(session) &&
                        session.Content.Visibility == Visibility.Collapsed,
                    TimeSpan.FromSeconds(5)))
            {
                throw new InvalidOperationException("The transfer fixture did not become inactive and idle.");
            }
            session.InactiveSince = DateTimeOffset.UtcNow - TimeSpan.FromMinutes(6);
            // WebView2 applies native visibility asynchronously after XAML selection.
            // Suspension can be declined until that transition reaches the renderer.
            var suspensionDeadline = DateTimeOffset.UtcNow + TimeSpan.FromSeconds(5);
            while (!await editorSessions.SuspendAsync(session, requireIdle: true) &&
                DateTimeOffset.UtcNow < suspensionDeadline)
            {
                await Task.Delay(50);
            }
            ...
            if (ReferenceEquals(ViewModel.ActiveTab, session.ViewModel)) firstMoveFailures.Add("source retained tab presentation");
            if (!ReferenceEquals(destination.ViewModel.ActiveTab, session.ViewModel)) firstMoveFailures.Add("destination lost tab presentation");
            ...
            VerifyWindowViewModelsForSmoke(destination);
    ...
    private async Task RunTitleBarSmokeAsync()
    {
        ...
        try
        {
            ...
            first.IsDirty = true;
            var originalDisplayName = first.DisplayName;
            first.DisplayName = "Presentation smoke drawing";
            UpdateTabHeader(first);
            UpdateStatusBar(first);
            if (StatusDocumentText.Text != first.DisplayName ||
                StatusStateText.Text != DesktopResources.Get("StatusUnsaved", "Unsaved changes") ||
                StatusDocumentText.GetBindingExpression(TextBlock.TextProperty) is null ||
                StatusStateText.GetBindingExpression(TextBlock.TextProperty) is null)
                throw new InvalidOperationException("Dynamic document and status text lost their bindings.");
            ...
            first.DisplayName = originalDisplayName;
            first.IsDirty = false;
            UpdateTabHeader(first);
            ...
            await VerifySettingsBindingsForSmokeAsync();
            HideSettingsPage();
            if (settingsPageVisible ||
            ...
            await File.WriteAllTextAsync(
                Path.Combine(AppContext.BaseDirectory, "titlebar-smoke.passed"),
                "passed");
        }
        catch (Exception exception)
        {
