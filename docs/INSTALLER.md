...
artifacts/installer/ExcalidrawDesktop-Setup-0.1.4-x64.exe.sha256
artifacts/installer/ExcalidrawDesktop-Setup-0.1.4-x64.exe.build.json
```

Version defaults to `ExcalidrawDesktop.App.csproj`. Use `-Version 0.1.5` to
...
```powershell
./ExcalidrawDesktop-Setup-0.1.4-x64.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG="setup.log"
```
...
./SmokeTests/Invoke-InstallerSmokeTest.ps1 `
  -InstallerPath ./artifacts/installer/ExcalidrawDesktop-Setup-0.1.4-x64.exe
```
